...
const SubmissionsModal: React.FC<SubmissionsModalProps> = ({
  ...
  const [openSubmissionsIds, setOpenSubmissionsIds] = useState<Set<number>>(
    new Set()
  );
  ...
  const reevaluateSubmission = async (id: number) => {
    ...
    try {
      ...
      setError({
        message: "Failed to reevaluate the submission. Please try again.",
      });
    } finally {
      setLoadingIds((prev) => {
  ...
  const stopEvaluation = async (id: number) => {
    ...
    try {
      ...
      ); // assuming a new submission object is returned
  ...
  const stopAllEvaluations = async () => {
    ...
    try {
      ...
      setError({
        message: "Failed to stop all evaluations. Please try again.",
      });
    }
  };
  ...
  return (
    ...
    >
          ...
          <div key={submission.id} className="submission-item">
            ...
            >
              ...
              <div className="btn-group">
                ...
                >
                  {loadingIds.has(submission.id) ? (
                    <AiOutlineRedo className="spinner" />
                  ) : (
                    <AiOutlineRedo />
                  )}
                </button>
                ...
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    downloadIndividualLog(submission);
                  }}
                >
                  <AiOutlineFileZip />
                </button>
      ...
      <div className="pagination-controls">
        ...
        <span>
          {currentPage} of {Math.ceil(submissions.length / submissionsPerPage)}
        </span>
        <button
          onClick={nextPage}
          disabled={
            currentPage === Math.ceil(submissions.length / submissionsPerPage)
          }
        >
          Next
        </button>
      ...
      <div className="footer">
        ...
        <button
          ...
          onClick={stopAllEvaluations}
        >
          Stop
        </button>
        <button
          title="extract in bulk all the students’ submissions logs in a zip file"
        ...
        <button
          title=" extract in bulk all the students’ submission metadata in a CSV file"
          className="Extract-button"
