...
const ConfigureModal: React.FC<InputModalProps> = ({
  assignment,
  isOpen,
  onClose,
  onSave,
  ...
  const handleSubmit = async () => {
    if (testMode) {
      ...
      try {
        ...
        formData.append(
          "students",
          JSON.stringify(selectedStudents.map((student) => student.id))
        );
        ...
        setError({
          message: "Failed to update the assignment. Please try again.",
        });
      } finally {
        setLoading(false);
      }
  ...
  const handleStudentSelection = (
    ...
    const selectedStudents = selectedOptions
      .map((option) => students.find((student) => student.id === option.value))
      .filter((student): student is Person => student !== undefined);
          ...
          <div className="file-row">
            {file ? (
              <div className="file-info-modal">
                ...
                <p>{file.name}</p>
                <div className="remove-file" onClick={handleFileRemove}>
                  &times;
                </div>
              </div>
            ) : null}
